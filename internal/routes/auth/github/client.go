...
func newGithubApiClient(tok *oauth2.Token, org string) *githubMiniClient {
...
func (mc *githubMiniClient) getUserInfo() (userInfo, error) {
	uri, err := url.JoinPath(apiUrl, "user")
	if err != nil {
		return userInfo{}, err
	}
...
func (mc *githubMiniClient) listTeams() ([]teamInfo, error) {
	uri, err := url.JoinPath(apiUrl, fmt.Sprintf("/orgs/%s/teams", mc.org))
	if err != nil {
		return nil, err
	}
...
func (mc *githubMiniClient) isUserMemberOfTeam(username string, team teamInfo) (bool, error) {
	uri, err := url.JoinPath(apiUrl, "/orgs/%s/teams/%s/memberships/%s",
